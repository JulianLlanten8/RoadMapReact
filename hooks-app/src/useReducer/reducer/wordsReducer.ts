...
export type ScrabbleWordsAction =
  | { type: 'SET_GUESS', payload: string }
  | { type: 'CHECK_ANSWER' }
  | { type: 'SKIP_WORD' }
  | { type: 'PLAY_AGAIN', payload: ScrambleWordState };

export const scrambleWordsReducer = (state: ScrambleWordState, action: ScrabbleWordsAction): ScrambleWordState => {
  switch (action.type) {
    case 'SET_GUESS':
      return {
        ...state,
    ...
    case 'CHECK_ANSWER': {
      ...
      if (state.currentWord === state.guess) {
        ...
        // Si ya no hay palabras, termina el juego
      ...
      // Si falla, suma error y termina si supera el máximo
      return {
        ...state,
        guess: '',
